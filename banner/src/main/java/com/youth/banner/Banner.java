...
public class Banner extends FrameLayout implements OnPageChangeListener {
    ...
    private List<Object> mImageUrlList;
    ...
    public Banner setImages(List<?> imageUrls) {
        mImageUrlList.addAll(imageUrls);
    ...
    public Banner start() {
        setBannerStyleUI();
        setImageList(mImageUrlList);
        setData();
        return this;
    }

    private void setTitleStyleUI() {
        if (mTitleList.size() != mImageUrlList.size()) {
        ...
        if (mTitleList != null && mTitleList.size() > 0) {
            bannerTitle.setText(mTitleList.get(0));
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            ...
            if (mImageLoader != null) {
                imageView = mImageLoader.createImageView(context, url);
            }
            if (imageView == null) {
...
//        Log.i(tag, ev.getAction() + "--" + isAutoPlay);
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
            ...
            LinearLayout.LayoutParams Unselectedparams = new LinearLayout.LayoutParams(mIndicatorWidth, mIndicatorHeight);
        ...
        switch (bannerStyle) {
            ...
            case BannerConfig.NUM_INDICATOR_TITLE:
                ...
                bannerTitle.setText(mTitleList.get(position - 1));
                break;
            case BannerConfig.CIRCLE_INDICATOR_TITLE:
            case BannerConfig.CIRCLE_INDICATOR_TITLE_INSIDE:
                bannerTitle.setText(mTitleList.get(position - 1));
