...
public class FaceManager {
    ...
    public static void handlerEmojiText(TextView comment, String name, String content, String time, boolean typing) {
        SpannableStringBuilder sb = new SpannableStringBuilder();
        StringBuilder replyName = new StringBuilder();
        if (!TextUtils.isEmpty(name)) {
            replyName.append("回复 ").append(name).append(" :  ");

            sb.append(replyName);
            ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(0x99101010);
            sb.setSpan(foregroundColorSpan, 3, 3 + name.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        sb.append(content);
        String regex = "\\[(\\S+?)\\]";
        Pattern p = Pattern.compile(regex);
        ...
        ForegroundColorSpan foregroundColorSpan = new ForegroundColorSpan(0x99101010);
        sb.setSpan(foregroundColorSpan, (replyName + content).length(), (replyName + "\u3000" + content + time).length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
